...
calc.flowdir.uphill <- function(elev, seed = NULL){
	#if(uphill) flowdir returns the reverse 'flow direction' (of water), i.e.
...
calc_flow_path <- function(flowdir, pathlimits = NULL, point) {
	...
	return(list(path=path, limited_TF=limited_TF, limit_id=limit_id))
}
...
calc.MigrationRoutes_EstimateFlowpaths <- function(elev, flowdir=NULL, patches, end_toLeft, seed = NULL){
	...
	if(end_toLeft){
		...
		paths_success_TF <- as.list(rep(TRUE, times=length(paths)))
				...
				if((ntemp <- raster::cellStats(i_vegPatch, 'sum')) > 0){ #test in case the entire patch lies on the y-border where flowdir cells are NA
					...
					if((tally[id, "TestedFlowpaths_N"] <- length(i_vegPatch_sample)) > 0){
						for(ip in seq_along(i_vegPatch_sample)){ #Loop through every sampled cell within patch
							...
							OutGood <- as.numeric(any((temp1 <- sp::coordinates(path[length(path)])[1]) <= xlims[1])) #only lower x-boundary is good
							OutBad <- as.numeric(any(temp1 >= xlims[2]) || any((temp2 <- sp::coordinates(path[length(path)])[2]) <= ylims[1]) || any(temp2 >= ylims[2]))
							paths[[si]] <- path
							paths_success_TF[[si]] <- OutGood || OutBad
							tally[id, "GoodMigration_N"] <- tally[id, "GoodMigration_N"] + OutGood
							tally[id, "BadMigration_N"] <- tally[id, "BadMigration_N"] + OutBad
							tally[id, "SuccessfulFlowpaths_N"] <- tally[id, "SuccessfulFlowpaths_N"] + paths_success_TF[[si]]
							si <- si + 1
						}
					}
				} else {
	...
	return(list(tally=tally, paths=paths, paths_success_TF=paths_success_TF))
}
...
## TODO(drs): replace 'get.2ndPatchInPaths' with 'get.PatchPrevioustoIDInPaths'
	...
	if(length(lpatch3) > 0){
		patchIDs_GoodMigration <- get.targetPatchIDInPaths(patchIDs_crossedByPaths[apply(sapply(lpatch3, FUN=function(id) get.PathsWithTargetPatchID(patchIDs_crossedByPaths, id)), 1, any) & unlist(paths_success_TF)])
	} else {
	...
	return(list(patchIDs_BadMigration=patchIDs_BadMigration, patchIDs_GoodMigration=patchIDs_GoodMigration))
}
...
calc.RemoveBadMigration_fromVeg <- function(patches4, patches8, patch4IDs_remove){
	...
	return(list(grid=veg, patches8=patched8, patches4=patched4, patchID_removed=patch4IDs_remove))
}
